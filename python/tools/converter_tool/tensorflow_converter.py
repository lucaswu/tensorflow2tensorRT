...
class TensorflowConverter(base_converter.ConverterInterface):
    ...
    padding_mode = {
        b'VALID': PaddingMode.VALID,
        b'SAME': PaddingMode.SAME,
        b'FULL': PaddingMode.FULL
    }
    pooling_type_mode = {
    ...
    def infer_tensor_shape_old(self, tensor):
        inferred_tensor_shape = tensor.shape.as_list()
        inferred_success = True
        for _, dim in enumerate(inferred_tensor_shape):
            if dim is None:
    ...
    def convert_activation(self, tf_op):
        ...
        type_arg.i = self.activation_type[tf_op.type].value
    ...
    def convert_resize_bicubic(self, tf_op):
        ...
        size_value = tf_op.inputs[1].eval().astype(np.int32)
        src_shape = tf_op.inputs[0].shape
        src_height = int(src_shape[1])
        dst_height = size_value[0]
        resize_scale = dst_height/src_height
        size_scale = op.arg.add()
        size_scale.name = 'resize_scale'
        size_scale.i = int(resize_scale) 
    ...
    def convert_reshape(self, tf_op):
        ...
        shape = tf_op.inputs[1].eval().astype(np.int32)
        # print("reshape : ")
        # print(shape)
        reshape_arg = op.arg.add()
        reshape_arg.name = MaceKeyword.mace_dims_str
        reshape_arg.ints.extend(shape)
    ...
    def convert_subpixel(self,tf_op):
        ...
        op.output.extend([tf_output.name for tf_output in self._subpixel_Ops["output"].outputs])
        ...
        ConverterUtil.add_data_format_arg(op, DataFormat.NHWC)
        # print("1111--convert_subpixel",op)

...
from converter_tool.base_converter import DataFormat
from converter_tool.base_converter import DeviceType
from converter_tool.base_converter import EltwiseType
from converter_tool.base_converter import FilterFormat
from converter_tool.base_converter import MaceKeyword
from converter_tool.base_converter import MaceOp
from converter_tool.base_converter import PaddingMode
from converter_tool.base_converter import TransformerRule
from convert_util import calculate_image_shape
from convert_util import mace_check
from convert_util import OpenCLBufferType
from quantization import quantize_util
...
class Transformer(base_converter.ConverterInterface):
    ...
    def add_in_out_tensor_info(self):
        ...
        base_shape = self._option.base_shape

        for input_node in self._option.input_nodes.values():
            input_info = net.input_info.add()
            input_info.name = input_node.name

            scale_shape = [ float(a/b) for a,b in zip(input_node.shape,base_shape)]
            
            # print("scale_shape=",scale_shape,"base:",base_shape,"input:",input_node.shape)

            input_info.dims.extend(scale_shape)

            # print("input_node:",input_node.name,type(input_node))
            # net.input.extend([input_node.name])
        # return
    ...
    def fold_sub_pixel(self):
        self.construct_ops_and_consumers()

        print("fold reshape-transpose-reshape to sub pixel")
        net = self._model
        for op in net.op:
            if op.type == MaceOp.Transpose.name:
                former_reshape_op = self._producer[op.input[0]]
                if former_reshape_op.type != MaceOp.Reshape.name:
                    continue
                next_reshape_op = self._consumers[op.output[0]][0]
                if next_reshape_op.type != MaceOp.Reshape.name:
                    continue

                #create subpixel layer
                net = self._model
                index = op.name.rfind('/')
                input_name = former_reshape_op.input[0]
                op_def = net.op.add()
                op_def.name = op.name[0:index]+'/subpixel'
                output_name = op_def.name
                op_def.type = MaceOp.Subpixel.name
                op_def.input.extend([input_name])
                op_def.output.extend([output_name])

                former_op = self._producer[former_reshape_op.input[0]]
                inputDims = former_op.output_shape[0].dims
                outputDims = next_reshape_op.output_shape[0].dims
                scale = math.sqrt(inputDims[3]/outputDims[3])
                subpiexl_scale_arg = op_def.arg.add()
                ...
                for i in range(len(former_op.output)):
                    # print("former_op.output[i]=",former_op.output[i],"former_reshape_op.input[0]=",former_reshape_op.input[0])
                    if operator.eq(former_op.output[i], former_reshape_op.input[0]):
                        # print('into sub1')
                        former_op.output[i] = input_name

                #if self._consumers.has_key(next_reshape_op.output[0]):
                if next_reshape_op.output[0] in self._consumers:
                    next_op = self._consumers[next_reshape_op.output[0]][0]
                    for i in range(len(next_op.input)):
                        if operator.eq(next_op.input[i], next_reshape_op.output[0]):
                            # print('into sub2')
                            next_op.input[i] = output_name

                img_shape = calculate_image_shape(
                    OpenCLBufferType.IN_OUT_CHANNEL, output_shape.dims)
                # update OpenCL max image size
                self._opencl_max_image_size[0] = max(self._opencl_max_image_size[0],
                                             img_shape[0])
                self._opencl_max_image_size[1] = max(self._opencl_max_image_size[1],
                ...
                self.safe_remove_node(
                    next_reshape_op, op, remove_input_tensor=True)
                # remove transpose
                self.safe_remove_node(
                    op, former_reshape_op, remove_input_tensor=True)
                # remove producer reshape
                self.safe_remove_node(former_reshape_op, self._producer.get(
                    former_reshape_op.input[0], None), remove_input_tensor=True)
    ...
    def fold_pad(self):
        ...
        for op in net.op:
            if op.type == MaceOp.Pad.name:
                next_op = self._consumers[op.output[0]][0]
                # print("next_reshape_op.name=",next_reshape_op.type,MaceOp.DepthwiseConv2d.name)
                if not (next_op.type == MaceOp.Conv2D.name or next_op.type == MaceOp.DepthwiseConv2d.name):
                    continue

                input_name = op.input[0]
                next_op.input[0] = op.input[0]

                # output_name = next_op.output[0]

                # op_def = net.op.add()
                # op_def.name = next_op.name
                # op_def.type = next_op.type

                # # for name in next_reshape_op.input:
                # #     print("name=",name)
                # # print("next_reshape_op=",next_reshape_op)

                # op_def.input.extend([input_name])
                # op_def.input.extend([next_op.input[1]])
                # op_def.input.extend([next_op.input[2]])


                # op_def.output.extend([output_name])

                # output_shape = op_def.output_shape.add()
                # output_shape.dims.extend(next_op.output_shape[0].dims)

                # print("op_def=",op_def)
                self._model.op.remove(op)
                # self.safe_remove_node(op, self._producer.get(
                #     op.input[0], None), remove_input_tensor=True)
                # sys.exit()
